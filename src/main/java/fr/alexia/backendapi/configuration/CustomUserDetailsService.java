...
public class CustomUserDetailsService implements UserDetailsService {
    ...
    @Autowired
    private IUserRepository userRepository;//accéder aux données des utilisateurs dans la base de données
    @Autowired
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        ...
        // informations utilisateur sont mappées à un objet User de Spring Security en utilisant org.springframework.security.core.userdetails.User.
        return new org.springframework.security.core.userdetails.User(
                user.getUsername(),
                user.getPassword(),
                getGrantedAuthorities(user));
    }
    //Les rôles ou autorisations de l'utilisateur sont ajoutés à la liste des autorisations accordées (GrantedAuthority) 
    private List<GrantedAuthority> getGrantedAuthorities(InternalUser user) {
        List<GrantedAuthority> authorities = new ArrayList<>();
        // si l'utilisateur a une location (rental), il est considéré comme un propriétaire (ROLE_OWNER), sinon il est considéré comme un utilisateur normal (ROLE_USER)
